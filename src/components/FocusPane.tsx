 ...
 *
 * A raised plate holding one lit recess: the page inside is the only thing
 * meant to be looked at, so everything around it stays flat.
 */
export function FocusPane({
  ...
  return (
    <div className="table-plane flex flex-col overflow-hidden">
      <div className="flex items-baseline justify-between gap-3 border-b border-edge px-4 py-3">
        <span className="data text-[12.5px] font-medium text-ink-quiet">{label}</span>
        {caption && <span className="truncate text-[12.5px] text-ink-faint">{caption}</span>}
      </div>

      <div className="recess relative flex min-h-[300px] flex-1 items-center justify-center overflow-hidden p-5 lg:min-h-[440px]">
        {src ? (
          <div className="relative" style={{ lineHeight: 0 }}>
            ...
            <img
              ...
              alt=""
              draggable={false}
              className="select-none rounded-[3px] bg-sheet shadow-[var(--lift-2)]"
              style={{
          ...
          <p className="max-w-[24ch] text-center text-[13.5px] leading-relaxed text-ink-quiet">
      ...
      {actions && (
        <div className="flex flex-wrap items-center gap-1 border-t border-edge px-2.5 py-2">
          {actions}
        </div>
      )}
    </div>
